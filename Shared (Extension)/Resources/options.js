  ...
  function render(data) {
    ...
    $('services').innerHTML = rows.map(([id, service]) => {
      ...
      return `<div class="service ${config.enabled ? '' : 'disabled'}" data-service="${esc(id)}">
        ...
        <div class="action-buttons">${healthLabel(config, service)}<button class="icon ${favorite ? 'active' : ''}" data-action="favorite" title="${favorite ? esc(t('unpin')) : esc(t('pin'))}" aria-label="${favorite ? esc(t('unpin')) : esc(t('pin'))}">${favorite ? '★' : '☆'}</button><button data-action="best">${esc(t('selectBest'))}</button><button data-action="health">${esc(t('check'))}</button><button data-action="custom">Custom…</button>${isCustom ? '<button data-action="removeCustom">Remove</button>' : ''}</div>
      </div>`
    }).join('')
